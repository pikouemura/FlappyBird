...
//  FlappyBird
//
//  Created by 上村 宙生 on 2016/06/23.
...
class GameScene: SKScene,SKPhysicsContactDelegate {
    ...
    //効果音
    let musicCoin = SKAction.playSoundFileNamed("coin.mp3", waitForCompletion: true)
    let musicJump = SKAction.playSoundFileNamed("jump.mp3", waitForCompletion: true)
    ...
    func setupScoreLabel() {
        ...
        itemScoreLabelNode.fontSize = 15
        itemScoreLabelNode.position = CGPoint(x: 10, y: self.frame.size.height - 45)
        ...
        bestScoreLabelNode.fontSize = 15
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.runBlock({
            ...
            //アイテムのy軸下限
            let item_lowest_y = UInt32(self.frame.size.height * 1 / 4)
            // 1〜random_y_rangeまでのランダムな整数を生成
            let random_y = arc4random_uniform( UInt32(self.frame.size.height * 2 / 4) )
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
            ...
            // スプライトに物理演算を設定する
            item.physicsBody = SKPhysicsBody(circleOfRadius: item.size.height / 2.0) // ←追加
            
            // 衝突の時に動かないように設定する
            item.physicsBody?.dynamic = false // ←追加
        ...
        let startRepeatForeverAnimation = SKAction.sequence([SKAction.waitForDuration(1), repeatForeverAnimation])
        
        runAction(startRepeatForeverAnimation)
    }
    func setupBird() {
        // 鳥の画像を2種類読み込む
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        print("bodyA"+String(contact.bodyA.categoryBitMask))
        print("bodyB"+String(contact.bodyB.categoryBitMask))
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            self.runAction(musicCoin)
            
            //アイテムを消す
            let itemBody:SKPhysicsBody!
    ...
    func restart() {
        ...
        itemScoreLabelNode.text = String("Score:\(itemScore)") // ←追加
